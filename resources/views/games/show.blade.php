    ...
    <div class="container mx-auto mt-32 md:mt-20">
        ...
        <div class="bg-slate-200 opacity-90 mx-auto p-6 flex flex-col justify-center items-center mt-12 gap-y-4 rounded-lg w-10/12 md:w-1/2">
            ...
            <div class="flex justify-center items-center bg-white rounded-lg">
                <div class="flex-col justify-between items-center p-6 w-full">
                    <p class="font-bold text-end w-full text-lg md:text-4xl">{{$game->local->name}} <span class="bg-gray-500 rounded-full ml-2 p-2 text-white">{{$game->local_score}}</span></p>
                    <p class="text-lg font-semibold text-center pb-1">vs</p>
                    <p class="font-bold text-end text-lg md:text-4xl">{{$game->visitor->name}} <span class="bg-gray-500 rounded-full ml-2 p-2 text-white">{{$game->visitor_score}}</span></p>
                </div>
            </div>
