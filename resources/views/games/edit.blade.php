...
<div class="container mx-auto mt-32 md:mt-12">
    ...
    <div class="bg-slate-200 opacity-90 w-11/12 sm:w-10/12 md:w-9/12 lg:w-7/12 mx-auto p-2 flex flex-col justify-center items-center mt-12 mb-2 gap-y-2 rounded-lg">
        ...
        <p class="text-center text-2xl mb-1 font-bold">EDITAR PARTIDO</p>

        <form action="{{route('games.update', $game)}}" method="POST" class="flex flex-col items-center w-3/4 md:w-3/4 lg:w-8/12 xl:w-8/12 gap-y-2">
            ...
            <div class="container">
                ...
                <div class="flex flex-col pb-7 relative items-start">
                    <div class="flex">
                        <label for="gameweekid" class="mr-3">Jornada</label>
                ...
                <div class="flex flex-col pb-7 relative items-start">
                    <div class="flex">
                        ...
                        <input type="date" name="date" value="{{ old('date', $game->date) }}" class="col-span-4">
                    </div>
                    ...
                    <div class="flex flex-col sm:flex-row">
                        ...
                        <select name='local_team_id' id="localteamid" class="col-span-4">
                            ...
                            @foreach ($teams as $team)
                                <option value='{{ $team->id }}' {{ old('local_team_id', $game->local_team_id) == $team->id ? 'selected' : '' }}>{{ $team->name }}</option>
                            @endforeach
                        </select>
                    </div>
                ...
                <div class="flex flex-col pb-7 relative items-start">
                    <div class="flex">
                        ...
                        <input type='number' name='local_score' value='{{ old('local_score', $game->local_score) }}' min='0' max='99' placeholder='--' class="col-span-4">
                    </div>
                ...
                <div class="flex flex-col pb-7 relative items-start">
                    <div class="flex flex-col sm:flex-row">
                        ...
                        <select name='visitor_team_id' id="visitorteamid" class="col-span-4">
                            ...
                            @foreach ($teams as $team)
                                <option value='{{ $team->id }}' {{ old('visitor_team_id', $game->visitor_team_id) == $team->id ? 'selected' : '' }}>{{ $team->name }}</option>
                            @endforeach
                        </select>
                    </div>
                ...
                <div class="flex flex-col pb-7 relative items-start">
                    <div class="flex">
                        ...
                        <input type='number' name='visitor_score' value='{{ old('visitor_score', $game->visitor_score) }}' min='0' max='99' placeholder='--' class="col-span-4">
                    </div>
            ...
            <button type="submit" class="w-full lg:w-2/3 mx-auto p-2 rounded-md transition-colors duration-300 ease-in-out bg-white hover:bg-green-500 hover:text-white">ACTUALIZAR</button>
